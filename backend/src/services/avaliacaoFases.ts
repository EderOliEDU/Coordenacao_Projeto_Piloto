...
export async function ensureAvaliacaoFasesStructure() {
  ...
  await pool.query(`
    ...
    VALUES (1, '1ª Fase', DATE '2026-06-01', DATE '2026-07-03', true, 1, 'Fase inicial criada para preservar os registros ja existentes.')
...
export async function getFaseAtual(): Promise<AvaliacaoFase> {
  ...
  const result = await pool.query<{
    ...
    WHERE ativo = true
    ORDER BY
      CASE WHEN CURRENT_DATE BETWEEN data_inicio AND data_fim THEN 0 ELSE 1 END,
      data_inicio DESC,
      ordem DESC,
      id_fase DESC
    LIMIT 1
  ...
  const fase = result.rows[0]
  if (!fase) {
    throw new Error('Nenhuma fase de avaliacao ativa cadastrada')
  }
...
export async function listarFases() {
  ...
  const pool = getPgPool()
  const result = await pool.query<AvaliacaoFase>(
    `
    SELECT
      id_fase AS id,
      nome,
